<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('feeds.store') }}">
            ...
            <x-primary-button class="mt-4">{{ __('Feed') }}</x-primary-button>
        </form>
            ...
            <div class="p-6 flex space-x-2">
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-center">
                        <div>
                            <span class="text-gray-800">{{ $feed->user->name }}</span>
                            <small class="ml-2 text-sm text-gray-600">{{ $feed->created_at->format('j M Y, g:i a')
                                }}</small>
                            @unless ($feed->created_at->eq($feed->updated_at))
                        ...
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                ...
                                <form method="POST" action="{{ route('feeds.destroy', $feed) }}">
                                    @csrf
                                    @method('delete')
                                    <x-dropdown-link :href="route('feeds.destroy', $feed)"
                    ...
                    <p class="mt-4 text-lg text-gray-900">{{ $feed->message }}</p>
                </div>
            </div>
            @endforeach
        </div>
